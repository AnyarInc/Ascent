...
#include "ascent/direct/state.hpp"
...
namespace asc
{
   ...
   template <void(block::* func)(), class modules_t>
   ...
   {
      call_loop<&block::operator()>(blocks);
   }

   template <class modules_t>
   void update(modules_t& blocks, asc::block* run_first)
   {
      if (run_first) {
         (*run_first)();
      }
   ...
   {
      call_loop<&block::postprop>(blocks);
   }

   template <class modules_t>
   inline void postcalc(modules_t& blocks)
   {
      call_loop<&block::postcalc>(blocks);
   }
